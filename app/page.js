...
export default function WorkTracker() {
  ...
  const handleDeleteEntry = (month, indexToDelete) => {
    const updatedEntries = entries.filter((entry, index) => {
      const entryMonth = getMonthName(entry.date);
      return !(entryMonth === month && entriesByMonth[month].indexOf(entry) === indexToDelete);
    });
    ...
    <div className="p-6 max-w-3xl mx-auto space-y-6 bg-gradient-to-br from-sky-100 to-blue-200 rounded-xl shadow-xl">
      ...
      <div className="flex justify-end mb-2">
        {session?.user ? (
          ...
          >
            💾 Save your work hours to your account
          </button>
        )}
      </div>
        ...
        <CardContent className="p-6 space-y-4">
          ...
          <div className="flex flex-col gap-2">
            ...
            <Button
              ...
              className="w-full mt-4 bg-blue-600 hover:bg-blue-700 text-white text-lg font-medium rounded-md"
            >
              Add Entry
            </Button>
          </div>
      ...
      {Object.entries(entriesByMonth).map(([month, monthEntries], index) => {
        ...
        const totalEarned = monthEntries.reduce(
          (sum, e) => sum + e.earned,
          0
        );
        ...
        return (
          <Card
            key={index}
            className="bg-white shadow-lg border-2 border-green-200"
          >
            <CardContent className="p-6 space-y-4">
                  ...
                  <Card key={i} className="bg-gray-50 shadow-sm">
                    <CardContent className="p-4">
                      <div className="flex justify-between items-start text-blue-900">
                        ...
                        <button
                          onClick={() => handleDeleteEntry(month, i)}
                      ...
                      <div className="text-sm text-gray-600 mt-1">
                        Worked: {entry.hoursWorked.toFixed(2)} hrs | Earned: $
